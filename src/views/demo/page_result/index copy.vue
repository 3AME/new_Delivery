...
export default {
  ...
  methods: {
    solve (
      ...
      var isRouteMode = this.problem.num_node !== undefined
      if (!isRouteMode) {
      ...
      pipe(problem.num_node)
      pipe(problem.edges.length)
      for (var e of problem.edges) {
        pipe(e.u, e.v, e.w)
      }
      pipe(problem.customers.length)
      for (var demand of problem.customers) {
        pipe(demand)
      }
      pipe(problem.vehicles.length)
      for (var load of problem.vehicles) {
        pipe(load)
      }
      pipe(problem.distancePrior)
      pipe(problem.timePrior)
      pipe(problem.loadPrior)
      pipe(popsize, childsize, maxiter, mutationProb)
      solver.stdout.on('data', buffer => {
        console.log(buffer.toString())
        // eslint-disable-next-line no-eval
        // var solution = eval('(' + buffer.toString() + ')')
        console.log('solution=' + buffer.toString())
    ...
    showGraph () {
      ...
      for (var i = 0; i < problem.num_node; i++) {
        // var name = i.toString()
        var group = 1
        if (i === 0) {
          group = 2
      ...
      let svg = d3
        ...
        .attr('preserveAspectRatio', 'xMidYMid meet')
        .attr('viewBox', '0 0 ' + width + ' ' + height)
      console.log('width=' + width + ' height=' + height + ' svg=' + svg)
